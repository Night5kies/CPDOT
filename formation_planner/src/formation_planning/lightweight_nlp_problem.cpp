...
{
public:
  int num_sfc_cons, num_human_cons;
  LiomIPOPTInterfaceFm(double w_inf, const std::vector<Constraints> &profile, const std::vector<FullStates> &guess, const PlannerConfig &config, const std::vector<std::vector<std::vector<std::vector<double>>>>& corridor_cons, const std::vector<double>& height_cons)
    : w_inf_(w_inf), profile_(profile), guess_(guess), config_(config), corridor_cons_(corridor_cons), height_cons_(height_cons) {
    num_sfc_cons = 0;
    num_human_cons = 0;
    for (int i = 0; i < corridor_cons_.size(); i++) {
      for (int j = 0; j < corridor_cons_[i].size(); j++) {
        for (int k = 0; k < corridor_cons_[i][j].size(); k++) {
          num_sfc_cons++;
        }
      }
    }
    for (int j = 0; j < corridor_cons_[1].size(); j++) {
      for (int k = 0; k < corridor_cons_[1][j].size(); k++) {
        num_human_cons++;
      }
    }
  ...
  virtual bool get_bounds_info(Ipopt::Index n, Ipopt::Number* x_l, Ipopt::Number* x_u,
    ...
    for (int r = 0; r < nrows_; ++r) {
      for (int i = 0; i < num_robot; ++i) {
        x_l[1 + (0 + 7 * i) * nrows_ + r] = -inf;
        x_l[1 + (1 + 7 * i) * nrows_ + r] = -inf;
        x_l[1 + (2 + 7 * i) * nrows_ + r] = -inf;
        x_l[1 + (3 + 7 * i) * nrows_ + r] = config_.vehicle.min_velocity;
        x_l[1 + (4 + 7 * i) * nrows_ + r] = -config_.vehicle.phi_min;
        x_l[1 + (5 + 7 * i) * nrows_ + r] = -config_.vehicle.max_acceleration;
        x_l[1 + (6 + 7 * i) * nrows_ + r] = -config_.vehicle.omega_max;
      }
    }
    for (int r = 0; r < nrows_; ++r) {
      for (int j = 0; j < num_robot; ++j) {
        double val = (height_cons_[r + 1] == -1) ?
                      vvcm.xv2t * vvcm.xv2t :
                      3 * height_cons_[r + 1] * height_cons_[r + 1];
        x_l[1 + (num_robot * NVar + j) * nrows_  + r] = val;
      }
      for (int j = 0; j < num_robot * (num_robot - 2); ++j) {
        x_l[1 + (num_robot * NVar + num_robot + j) * nrows_ + r] = 0.0;
      }
    }
    // int offset2 = (offset + delta) * nrows_;
    for (int i = 0; i < 2 * num_sfc_cons; ++i) {
        x_l[1 + (num_robot * NVar + num_robot + num_robot * (num_robot - 2)) * nrows_ + cont] = -inf;
        cont++;
    }
    ...
    for (int r = 0; r < nrows_; ++r) {
      for (int i = 0; i < num_robot; ++i) {
        x_u[1 + (0 + 7 * i) * nrows_ + r] = inf;
        x_u[1 + (1 + 7 * i) * nrows_ + r] = inf;
        x_u[1 + (2 + 7 * i) * nrows_ + r] = inf;
        x_u[1 + (3 + 7 * i) * nrows_ + r] = config_.vehicle.max_velocity;
        x_u[1 + (4 + 7 * i) * nrows_ + r] = config_.vehicle.phi_max;
        x_u[1 + (5 + 7 * i) * nrows_ + r] = config_.vehicle.max_acceleration;
        x_u[1 + (6 + 7 * i) * nrows_ + r] = config_.vehicle.omega_max;
      }
    }
    for (int r = 0; r < nrows_; ++r) {
      for (int j = 0; j < num_robot; ++j) {
        x_u[1 + (num_robot * NVar + j) * nrows_ + r] = vvcm.xv2 * vvcm.xv2;
      }
      for (int j = 0; j < num_robot * (num_robot - 2); ++j) {
        x_u[1 + (num_robot * NVar + num_robot + j) * nrows_ + r] = inf;
      }
    }
    for (int i = 0; i < 2 * num_sfc_cons; ++i) {
        x_u[1 + (num_robot * NVar + num_robot + num_robot * (num_robot - 2)) * nrows_ + cont] = 0.0;
        cont++;
    }
    return true;
  }
  ...
  virtual bool get_starting_point(Ipopt::Index n, bool init_x, Ipopt::Number* x,
    ...
    for (int r = 0; r < nrows_; ++r) {
      for (int i = 0; i < num_robot; ++i) {
        x[1 + (0 + 7 * i) * nrows_ + r] = guess_[i].states[r+1].x;
        x[1 + (1 + 7 * i) * nrows_ + r] = guess_[i].states[r+1].y;
        x[1 + (2 + 7 * i) * nrows_ + r] = guess_[i].states[r+1].theta;
        x[1 + (3 + 7 * i) * nrows_ + r] = guess_[i].states[r+1].v;
        x[1 + (4 + 7 * i) * nrows_ + r] = guess_[i].states[r+1].phi;
        x[1 + (5 + 7 * i) * nrows_ + r] = guess_[i].states[r+1].a;
        x[1 + (6 + 7 * i) * nrows_ + r] = guess_[i].states[r+1].omega;
      }
    }
    for (int r = 0; r < nrows_; ++r) {
      for (int j = 0; j < num_robot; ++j) {
        x[1 + (num_robot * NVar + j) * nrows_ + r] = pow(guess_[(j+1)%num_robot].states[r+1].x - guess_[j].states[r+1].x, 2) + pow(guess_[(j+1)%num_robot].states[r+1].y - guess_[j].states[r+1].y, 2);
      }
      for (int j = 0; j < num_robot; ++j) {
        for (int p = 0; p < num_robot; p++) {
          if (p != j && (p + 1) % num_robot != j) {
            int Np = (p + 1) % num_robot;
            x[1 + (num_robot * NVar + num_robot + j) * nrows_ + r] = (guess_[j].states[r+1].x - guess_[p].states[r+1].x) * (guess_[p].states[r+1].y - guess_[Np].states[r+1].y)
                        + (guess_[j].states[r+1].y - guess_[p].states[r+1].y) * (guess_[Np].states[r+1].x - guess_[p].states[r+1].x);
          }
        }
      }
    }
    for (int r = 0; r < nrows_; ++r) {
      for (int j = 0; j < num_robot; j++) {
        auto vec_temp = config_.vehicle.GetDiscPositions(guess_[j].states[r+1].x, guess_[j].states[r+1].y, guess_[j].states[r+1].theta);
        for (int n = 0; n < corridor_cons_[j][r].size(); n++) {
          x[1 + (num_robot * NVar + num_robot + num_robot * (num_robot - 2)) * nrows_ + cont] = corridor_cons_[j][r][n][0] * vec_temp[0] + corridor_cons_[j][r][n][1] * vec_temp[1] - corridor_cons_[j][r][n][2];
          cont++;
        }
        for (int n = 0; n < corridor_cons_[j][r].size(); n++) {
          x[1 + (num_robot * NVar + num_robot + num_robot * (num_robot - 2)) * nrows_ + cont] = corridor_cons_[j][r][n][0] * vec_temp[2] + corridor_cons_[j][r][n][1] * vec_temp[3] - corridor_cons_[j][r][n][2];
          cont++;
        }
      }      
    }
  ...
  template<class T> bool eval_obj(Ipopt::Index n, const T *x, T& obj_value) {
    ...
    for(int i = 0; i < nrows_; i++) {
      // 获取两个机器人的位置和朝向
      // adouble x_b = x[1 + (1 * 7 + 0) * nrows_ + i];
      // adouble y_b = x[1 + (1 * 7 + 1) * nrows_ + i];
      // adouble theta_b = x[1 + (1 * 7 + 2) * nrows_ + i];
      // adouble x_a = x[1 + (2 * 7 + 0) * nrows_ + i];
      // adouble y_a = x[1 + (2 * 7 + 1) * nrows_ + i];
      // adouble theta_a = x[1 + (2 * 7 + 2) * nrows_ + i];

      // // 相对位置向量
      // adouble dx = x[1 + (2 * 7 + 0) * nrows_ + i] - x[1 + (1 * 7 + 0) * nrows_ + i];
      // adouble dy = x[1 + (2 * 7 + 1) * nrows_ + i] - x[1 + (1 * 7 + 1) * nrows_ + i];
      // adouble norm = sqrt(x[1 + (2 * 7 + 0) * nrows_ + i] - x[1 + (1 * 7 + 0) * nrows_ + i] * x[1 + (2 * 7 + 0) * nrows_ + i] - x[1 + (1 * 7 + 0) * nrows_ + i] + 
      // x[1 + (2 * 7 + 1) * nrows_ + i] - x[1 + (1 * 7 + 1) * nrows_ + i] * x[1 + (2 * 7 + 1) * nrows_ + i] - x[1 + (1 * 7 + 1) * nrows_ + i] + 1e-6);  // 防止除0

      // // 代价项1：A 在 B 朝向方向上
      // adouble dir_diff_x = x[1 + (2 * 7 + 0) * nrows_ + i] - x[1 + (1 * 7 + 0) * nrows_ + i] / (sqrt(x[1 + (2 * 7 + 0) * nrows_ + i] - x[1 + (1 * 7 + 0) * nrows_ + i] * x[1 + (2 * 7 + 0) * nrows_ + i] - x[1 + (1 * 7 + 0) * nrows_ + i] + 
      // x[1 + (2 * 7 + 1) * nrows_ + i] - x[1 + (1 * 7 + 1) * nrows_ + i] * x[1 + (2 * 7 + 1) * nrows_ + i] - x[1 + (1 * 7 + 1) * nrows_ + i] + 1e-6);) - cos(theta_b);
      // adouble dir_diff_y = x[1 + (2 * 7 + 1) * nrows_ + i] - x[1 + (1 * 7 + 1) * nrows_ + i] / (sqrt(x[1 + (2 * 7 + 0) * nrows_ + i] - x[1 + (1 * 7 + 0) * nrows_ + i] * x[1 + (2 * 7 + 0) * nrows_ + i] - x[1 + (1 * 7 + 0) * nrows_ + i] + 
      // x[1 + (2 * 7 + 1) * nrows_ + i] - x[1 + (1 * 7 + 1) * nrows_ + i] * x[1 + (2 * 7 + 1) * nrows_ + i] - x[1 + (1 * 7 + 1) * nrows_ + i] + 1e-6);) - sin(theta_b);
      // obj_value += pow(x[1 + (2 * 7 + 0) * nrows_ + i] - x[1 + (1 * 7 + 0) * nrows_ + i] / (sqrt(x[1 + (2 * 7 + 0) * nrows_ + i] - x[1 + (1 * 7 + 0) * nrows_ + i] * x[1 + (2 * 7 + 0) * nrows_ + i] - x[1 + (1 * 7 + 0) * nrows_ + i] + 
      // x[1 + (2 * 7 + 1) * nrows_ + i] - x[1 + (1 * 7 + 1) * nrows_ + i] * x[1 + (2 * 7 + 1) * nrows_ + i] - x[1 + (1 * 7 + 1) * nrows_ + i] + 1e-6)) - cos(x[1 + (1 * 7 + 2) * nrows_ + i]), 2) + pow(x[1 + (2 * 7 + 1) * nrows_ + i] - x[1 + (1 * 7 + 1) * nrows_ + i] / (sqrt(x[1 + (2 * 7 + 0) * nrows_ + i] - x[1 + (1 * 7 + 0) * nrows_ + i] * x[1 + (2 * 7 + 0) * nrows_ + i] - x[1 + (1 * 7 + 0) * nrows_ + i] + 
      // x[1 + (2 * 7 + 1) * nrows_ + i] - x[1 + (1 * 7 + 1) * nrows_ + i] * x[1 + (2 * 7 + 1) * nrows_ + i] - x[1 + (1 * 7 + 1) * nrows_ + i] + 1e-6)) - sin(x[1 + (1 * 7 + 2) * nrows_ + i]), 2);

      // // 代价项2：朝向尽可能一致
      // // adouble heading_diff_x = cos([1 + (2 * 7 + 2) * nrows_ + i]) - cos(x[1 + (1 * 7 + 2) * nrows_ + i]);
      // // adouble heading_diff_y = sin([1 + (2 * 7 + 2) * nrows_ + i]) - sin(thetx[1 + (1 * 7 + 2) * nrows_ + i]a_b);
      // obj_value += pow(cos(x[1 + (2 * 7 + 2) * nrows_ + i]) - cos(x[1 + (1 * 7 + 2) * nrows_ + i]), 2) + pow(sin(x[1 + (2 * 7 + 2) * nrows_ + i]) - sin(x[1 + (1 * 7 + 2) * nrows_ + i]), 2);

      // obj_value += 1.0 * J1 + 1.0 * J2;
    }
    // terminal constraints
    // for (int i = 0; i < guess_.size(); i++) {
...
private:
  ...
  const std::vector<std::vector<std::vector<std::vector<double>>>>& corridor_cons_;
...
bool LightweightProblem::SolveFm(
  double w_inf, const std::vector<Constraints> &profile, const std::vector<FullStates> &guess, std::vector<FullStates> &result, double &infeasibility, double& solve_time, const std::vector<std::vector<std::vector<std::vector<double>>>>& corridor_cons, std::vector<double>& height_cons) {
  double solver_st = GetCurrentTimestamp();

  Ipopt::SmartPtr<LiomIPOPTInterfaceFm> interface = new LiomIPOPTInterfaceFm(w_inf, profile, guess, *config_, corridor_cons, height_cons);
