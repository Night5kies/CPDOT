...
void TopologyPRM::findTopoPaths(Eigen::Vector2d start, Eigen::Vector2d end,
                                ...
                                vector<vector<Eigen::Vector2d>>& select_paths, double& total_time,
                                const double& rectangle_ratio) {
  ros::Time t1, t2;
  ...
  // build the graph
  graph = createGraph(start, end, rectangle_ratio);
...
list<GraphNode::Ptr> TopologyPRM::createGraph(Eigen::Vector2d start, Eigen::Vector2d end, const double& rectangle_ratio) {
...
Eigen::Vector2d TopologyPRM::getSample() {
  ...
  // pt(0) = rand_pos_x(eng_);
  // pt(1) = rand_pos_y(eng_);
